    ...
    try:
        ...
        for course in course_data['Courses'][0]['Subjects']:
            ...
            for index in range(1, 11):
                subjects_url=f"https://api.kipslms.com/api/v5/CandidateCourseService/GetCourseSubjectActivityDays?candidateSOSId=543907&subjectId={subject_id}&groupName=WEEK+{index}"
                subject_data = fetch_subjects(subjects_url, auth_token)
                # print(subject_data)
                # sys.exit()
                for week in subject_data:
                    ...
                    week_url = f"https://api.kipslms.com/api/v5/CandidateCourseService/GetCourseSubjectDayActivitites?candidateSOSId=543907&subjectId={subject_id}&sosDetailId={SOSDetailId}" 
                    ...
                    #print(Title," Prev title")
                        if day["ActivityType"]==1:
                            ...
                            notes_url=f"https://api.kipslms.com/api/v5/CandidateContentService/GetContentReading?contentId={ContentId}&sosDetailContentId={SOSDetailContentId}&CandidateSOSId=543907"
                            notes_data=fetch_subjects(notes_url, auth_token)
                            content=notes_data["Description"]
                            title=notes_data["Title"]
                            print(title)
                            index3+=1
                            path=f"Notes/{subject_Name}/{index3}-{Title}.pdf"
