...
def download_videos(video_list):
    ...
    for video in reversed(video_list):
        ...
        try:
            ...
            if not os.path.exists("PhyInSeconds/Physics"):
                os.makedirs("PhyInSeconds/Physics")
            ...
            if response.status_code == 200:
                ...
                with open(f"PhyInSeconds/Physics/{index}-{title}.mp4", 'wb') as f:
                    f.write(response.content)
                    index=index+1
                print(f"Video '{title}' downloaded successfully.")
            else:
                print(f"Failed to download video '{title}' from {url}.")
...
def make_login():
    ...
    response=session.get("https://physicsinseconds.com/api/course/mdcat-physics-advance?contents",headers=headers)
    ...
    for i in json_data['course']['videos']:
        try:
            sourceid = json_data['course']['videos'][i]['sourceid']
            if(sourceid==""):
                continue
            else:
                title = json_data['course']['videos'][i]['title']
                #print(sourceid)
                res = session.get(f"https://fast.wistia.com/embed/medias/{sourceid}.json")
                video_json = res.json()
                #print(video_json)
                # with open("example3.txt", "w") as file:
                #     file.write(json.dumps(video_json))
                #break
                try:
                    for video in video_json["media"]["assets"]:
                        if (video["type"] == 'mp4_video'):
                            print(video["url"])
                            data={
                                'url': video['url'],
                                'title': title
