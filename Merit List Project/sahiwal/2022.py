...
def download_file(url,rnum):
    try:
        ...
        if 'BIOLOGY' in soup.text:
            ...
            print(name)
            phy = int((f'{td_tags[81].text}'))
            chem = int((f'{td_tags[93].text}'))
            bio = int((f'{td_tags[105].text}'))

            marks1 = bio+phy+chem
            marks = int((f'{td_tags[124].text}'))
            bar()
...
with alive_bar(360001-300001) as bar:
    with ThreadPoolExecutor(max_workers=5) as executor:
        for i in range(300001,360001):
            sleep(0.1)
...
for task in as_completed(processes):
    ...
    if type(task.result()) is not str:
        #print(task.result())
        with open('results_2023sahiwal.csv','a',newline='') as new:
            count +=1
            print(count)
